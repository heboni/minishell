...
/*   Updated: 2022/10/07 23:36:45 by heboni           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ast_exec_pipe(t_btree *node)
{
	...
	(void) node;
	pipe_node = (t_ast_pipe *) ((t_node *) node->item)->data;
	pipe(pipe_node->pipefd);
	((t_node *) node->left->item)->fd_in = ((t_node *) node->item)->fd_in;
	((t_node *) node->left->item)->fd_out = pipe_node->pipefd[1];
	((t_node *) node->right->item)->fd_in = pipe_node->pipefd[0];
	//printf("%d >\n", ((t_node *) node->item)->fd_in);
	//printf("executing pipe: %d -> %d\n", pipe_node->pipefd[0], pipe_node->pipefd[1]);
	//printf("> %d\n", ((t_node *) node->item)->fd_out);
	//printf("\n");
}
