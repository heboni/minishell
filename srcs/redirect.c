...
/*   Updated: 2022/10/09 13:40:43 by heboni           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

char	*get_redirect_r_file(char **tokens, int *t_i, t_node *new)
{
	char	*f_name;
...
char	*get_redirect_rr_file(char **tokens, int *t_i, t_node *new)
{
	char	*f_name;
...
char	*get_redirect_l_file(char **tokens, int *t_i, t_node *new)
{
	char	*f_name;
...
char	*get_redirect_ll_file(char **tokens, int *t_i, t_node *new)
{
	char	*f_name;
...
{ //может мне не нужно сохранять список файлов, сразу открывать его и возвращать наиболее актуальный
	...
	if (ft_strcmp(tokens[*t_i], ">") == 0 && !is_in_exception_indexes(msh_ctx, *t_i))
	{
		...
		new->r_f = get_redirect_r_file(tokens, t_i, new);
	}
	else if (ft_strcmp(tokens[*t_i], ">>") == 0 && !is_in_exception_indexes(msh_ctx, *t_i))
	{
		if (new->rr_f)
			free(new->rr_f);
		new->rr_f = get_redirect_rr_file(tokens, t_i, new);
	}
	else if (ft_strcmp(tokens[*t_i], "<") == 0 && !is_in_exception_indexes(msh_ctx, *t_i))
	{
		if (new->l_f)
			free(new->l_f);
		new->l_f = get_redirect_l_file(tokens, t_i, new);
	}
	else if (ft_strcmp(tokens[*t_i], "<<") == 0 && !is_in_exception_indexes(msh_ctx, *t_i))
	{
		if (new->ll_f)
			free(new->ll_f);
		new->ll_f = get_redirect_ll_file(tokens, t_i, new); //TO BE DONE
	}
	else
		return (0);
	return (1);
